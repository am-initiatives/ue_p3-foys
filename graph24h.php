...
for( $i = 72; $i > 0 ; $i-- )
{
		...
		$date_inf->modify( "-$i hour" );
		$date_inf_req = $date_inf->format( 'H' );
$labelx[]=$date_inf_req."h";
}	
...
//$graph->xgrid->SetColor( $MAINetage['couleur2'] );
...
{
	for( $i = 72; $i > 0 ; $i-- ) // une boucle par heure
	{
		$j=$i-1;
		$date_inf = new DateTime;
		$date_inf->modify( "-$i hour" );
		...
		$date_sup->modify( "-$j hour" );
		$date_sup_req = $date_sup->format( 'Y-m-d H:i:s' );



		$req = $bdd->prepare( "SELECT * FROM operations WHERE operation_type = 'bucquage' AND operation_annulee = 0 AND operation_etage = ? AND operation_time >= ? AND operation_time < ?" );
